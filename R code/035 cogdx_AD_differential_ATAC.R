...
Maindm <- model.matrix(~0 + cog_path + age + pmi)
...
options(na.action='na.pass')
Maindm <- model.matrix(~0 + ad_adnc + age + pmi)
...
###continuous gpath
options(na.action='na.pass')
Maindm <- model.matrix(~0 + gpath + age + pmi)
rownames(Maindm) <- rownames(mdat)
colnames(Maindm)
Maindm <- Maindm[complete.cases(Maindm),]
...
options(na.action='na.pass')
Maindm <- model.matrix(~0 + cogn_global + age + pmi)
rownames(Maindm) <- rownames(mdat)
colnames(Maindm)
Maindm <- Maindm[complete.cases(Maindm),]
